...
public:
	...
	*/
	...
	/*! Add a uniform time sampling with the specified time per cycle and the start time.
		It will only create a new time sampling if any of those created yet is matching the input value.
	\param in_dTimePerCycle Time per cycle
	\param in_dStartTime Start time
	\return ::EResult_Success on success, please see ::EAbcResult for more return code information
	*/
	...
	*/
	...
	/*! Add a uniform time sampling with the specified time per cycle and the start time.
		It will create a new time sampling if any of those created yet is matching the input value, otherwise, it will reuse the index matching the input values.
	\param in_dTimePerCycle Time per cycle
	\param in_dStartTime Start time
	\return The time sampling index
	*/
	virtual uint32_t AddTimeSamplingAndReturnIndex( double in_dTimePerCycle, double in_dStartTime ) = 0;
};
