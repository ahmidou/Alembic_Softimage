...
public:
	...
	EAbcResult SetTimeSampling( double in_dTimePerCycle, double in_dStartTime, uint32_t /*in_uiTimeSamplingIndex*/ )
	{
		// From the Compound Header, get its time sampling object and set it with input values
		Alembic::Abc::PropertyHeader lOHeader = m_CompoundProperty.getHeader();

		Alembic::AbcCoreAbstract::TimeSamplingPtr l_pTimeSampling = lOHeader.getTimeSampling();

		if( l_pTimeSampling )
			*l_pTimeSampling = Alembic::AbcCoreAbstract::TimeSampling( in_dTimePerCycle, in_dStartTime );
		else
			; // Unfortunately, the current API doesn't allow creating a TimeSampling object on a PropertyHeader
...
public:
	...
	virtual EAbcResult SetTimeSampling( double in_dTimePerCycle, double in_dStartTime, uint32_t in_uiTimeSamplingIndex )
	{
		return CAbcOCompoundPropertyBase::SetTimeSampling( in_dTimePerCycle, in_dStartTime, in_uiTimeSamplingIndex );
	}
