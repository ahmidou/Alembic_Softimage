...
{
	AddTimeSamplingAndReturnIndex( in_dTimePerCycle, in_dStartTime );
	return EResult_Success;
}

uint32_t CAbcOArchive::AddTimeSamplingAndReturnIndex( double in_dTimePerCycle, double in_dStartTime )
{
	return m_OArchive.addTimeSampling( Alembic::AbcCoreAbstract::TimeSampling( in_dTimePerCycle, in_dStartTime ) );
}
