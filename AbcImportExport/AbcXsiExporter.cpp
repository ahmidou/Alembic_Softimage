...
void FindAbcGeomParamType( EAbcGeomScope& out_scope, EAbcDataTraits& out_traits, ICEAttribute& in_attr )
{
	...
	case XSI::siICENodeDataVector4:		out_traits = EDataTraits_V4f; break;	// Alembic doesn't include V4f, extended by AbcFramework
	...
	case XSI::siICENodeDataRotation:	out_traits = EDataTraits_Rotf; break;
	...
	if ( cameraObj.IsValid() )
	{
		...
		if ( spAbcCamera->CreateCameraSample( &spSample ) == EResult_Success )
		{
			...
			if( !bOrtho )
			{
				...
				double dPixelRatio = currentCamera.GetParameterValue( L"pixelratio" );
				...
				spSample->SetLensSqueezeRatio( dPixelRatio );
...
{
	double dFPS = GetObjectFPS();
	uint32_t l_uiTimeSamplingIndex = m_pExporter->AddTimeSampling( dFPS, m_pExporter->GetStartTime() );
			...
			if ( attr.GetDataType() & SUPPORTED_TYPES )
			{
				...
				if ( attr.GetDataType() != XSI::siICENodeDataShape )
				{
					...
					if ( attr.GetStructureType() == XSI::siICENodeStructureSingle )
					{
						...
						spProp->SetTimeSampling( dFPS, m_pExporter->GetStartTime(), l_uiTimeSamplingIndex );
					...
					else if ( attr.GetStructureType() == XSI::siICENodeStructureArray )
					{
						...
						spArray2DProp->SetTimeSampling( dFPS, m_pExporter->GetStartTime(), l_uiTimeSamplingIndex );
				...
				else
				{
					...
					spShapeProp->SetTimeSampling( dFPS, m_pExporter->GetStartTime(), l_uiTimeSamplingIndex );
...
XSI::CStatus ExportTreeNode::CreatePointCloud( IAbcOPoints** out_ppPoints, XSI::SIObject in_obj, IAbcOObject* in_pParent, const XSI::CString& in_csName )
{
	...
	double dFPS = GetObjectFPS( );
	spAbcPoints->SetTimeSampling( dFPS, m_pExporter->GetStartTime() );
...
uint32_t AbcXsiExporter::AddTimeSampling( double in_dTimePerCycle, double in_dStartTime )
{
	return m_spOArchive->AddTimeSamplingAndReturnIndex( in_dTimePerCycle, in_dStartTime );
}
