...
SICALLBACK AbcExport_Execute( CRef& in_ctxt )
{
	...
	CStatus status;
	{
		if( l_lStart > l_lEnd )
		{
			l_app.LogMessage( XSI::CUtils::Translate( L"The specified start value is invalid because it is greater than the specified end value.", L"ABCIMPORTEXPORT" ), siWarningMsg );
			return CStatus::InvalidArgument;
		}
		...
		LONG l_lPlayCtrlIn = (LONG)l_playctrl.GetParameterValue( L"In" );
		if( l_lStart < l_lPlayCtrlIn )
		{
			l_app.LogMessage( XSI::CUtils::Translate( L"The specified start value is invalid because it is smaller than the Play Control's Frame In parameter value.", L"ABCIMPORTEXPORT" ), siWarningMsg );
			return CStatus::InvalidArgument;
		}

		LONG l_lPlayCtrlOut = (LONG)l_playctrl.GetParameterValue( L"Out" );
		if( l_lEnd > l_lPlayCtrlOut )
		{
			l_app.LogMessage( XSI::CUtils::Translate( L"The specified end value is invalid because it is greater than the Play Control's Frame Out parameter value.", L"ABCIMPORTEXPORT" ), siWarningMsg );
			return CStatus::InvalidArgument;
		}
...
SICALLBACK ExportAbcOptions_PPGEvent( const CRef& in_ctxt )
{
	...
	else if ( eventID == PPGEventContext::siParameterChange )
	{
		...
		if( paramName == L"StartFrame" || paramName == L"EndFrame" )
		{
			Project l_prj = app.GetActiveProject();
			CRefArray l_proplist = l_prj.GetProperties();
			Property l_playctrl = l_proplist.GetItem( L"Play Control" );

			LONG l_StartFrameParam = (LONG)prop.GetParameterValue( L"StartFrame" );
			LONG l_EndFrameParam = (LONG)prop.GetParameterValue( L"EndFrame" );

			if( paramName == L"StartFrame" )
			{
				LONG l_lPlayCtrlIn = (LONG)l_playctrl.GetParameterValue( L"In" );
				if( l_StartFrameParam < l_lPlayCtrlIn )
					prop.PutParameterValue( L"StartFrame", l_lPlayCtrlIn );

				if( l_StartFrameParam > l_EndFrameParam )
					prop.PutParameterValue( L"StartFrame", l_EndFrameParam );
			}
			else if( paramName == L"EndFrame" )
			{
				LONG l_lPlayCtrlOut = (LONG)l_playctrl.GetParameterValue( L"Out" );
				if( l_EndFrameParam > l_lPlayCtrlOut )
					prop.PutParameterValue( L"EndFrame", l_lPlayCtrlOut );

				if( l_EndFrameParam < l_StartFrameParam )
					prop.PutParameterValue( L"EndFrame", l_StartFrameParam );
			}
		}
	}
	else if ( eventID == PPGEventContext::siOnInit ||
			  eventID == PPGEventContext::siOnClosed )
	{
		CustomProperty prop = ctxt.GetSource();
