...
void BuildImportTree( IAbcIObject* pObject, AbcImportTreeNode* pImportNode )
{
	...
	for ( size_t i = 0; i < numChildren; i++ )
	{
		...
		else if( spObj->GetType() == EIObject_Subd )
		{
			g_ImportLog.Log( true, siWarningMsg, "The object '%s', of type ISubD, is not supported by the Softimage Alembic Importer plug-in. Please re-export the original object without subdivision and try to re-import it in Softimage (e.g. in Maya, turn off the 'Sub Division Mesh' attribute on the object).", spObj->GetFullName() );
		}
